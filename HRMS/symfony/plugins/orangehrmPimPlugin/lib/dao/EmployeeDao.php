...
class EmployeeDao extends BaseDao {
	...
	public function getEmployeeCertifications($empNumber, $certificationCode = null) {
        try {
            $q = Doctrine_Query::create()
                            ->from('EmployeeCertification es')
                            ->where('es.emp_number = ?', $empNumber);

            if (!is_null($certificationCode)) {
                $q->andwhere('es.certification_id = ?', $certificationCode);
                return $q->fetchOne();
            }
	...
	public function saveEmployeeCertification(EmployeeCertification $empCertification) {
		
		try {
			
			$empCertification->save();
    ...
    public function deleteEmployeeSkills($empNumber, $entriesToDelete = null) {
	 ...
	 public function deleteEmployeeCertifications($empNumber, $entriesToDelete = null) {
        ...
        try {
            ...
            if (is_array($entriesToDelete) && count($entriesToDelete) > 0) {                
                $q->whereIn('Certification_id', $entriesToDelete);                
            }
     ...
     * @throws DaoException
     */
    public function getEmployeeProjects($empNumber, $projectId = null) {
        try {
            $q = Doctrine_Query::create()
                            ->from('EmployeeProject eprj')
                            ->leftJoin('eprj.Project prj')
                            ->where('eprj.empNumber = ?', $empNumber);
             if (!is_null($projectId)) {
                $q->andwhere('eprj.projectId = ?', $projectId);
                return $q->fetchOne();
            }
            
            $q->orderBy('prj.name ASC');
     ...
     * @throws DaoException
     */
    public function getEmployeeCustomer($empNumber, $customerId = null) {
        try {
            $q = Doctrine_Query::create()
                            ->from('EmployeeProject eprj')
                            ->leftJoin('eprj.Customer cust')
                            ->where('eprj.empNumber = ?', $empNumber);
             if (!is_null($customerId)) {
                $q->andwhere('eprj.customerId = ?', $customerId);
                return $q->fetchOne();
            }
            
            $q->orderBy('cust.name ASC');
     ...
     * @throws DaoException
     */
    public function getEmployeeProject($empNumber, $projectId = null) {
        try {
            $q = Doctrine_Query::create()
                            ->from('EmployeeProject eprj')
                            ->leftJoin('eprj.Project proj')
                            ->where('eprj.empNumber = ?', $empNumber);
             if (!is_null($projectId)) {
                $q->andwhere('eprj.projectId = ?', $projectId);
                return $q->fetchOne();
     ...
     * @returns EmployeeSkill
     */
    public function saveEmployeeProject(EmployeeProject $empProject) {
        try {
            $empProject->save();
            return $empProject;
        // @codeCoverageIgnoreStart
     ...
     * @throws DaoException
     */
    public function deleteEmployeeProject($empNumber, $entriesToDelete = null) {
        
        try {
            
            $q = Doctrine_Query::create()->delete('EmployeeProject')
                                         ->where('emp_number = ?', $empNumber);
            
            if (is_array($entriesToDelete) && count($entriesToDelete) > 0) {                
                $q->whereIn('projectId', $entriesToDelete);                
            }
     ...
     * @throws DaoException
     */
    public function getEmployeeIdList($excludeTerminatedEmployees = false, $excludeConsultant = false) {
     ...
     * @throws DaoException
     */
    public function getEmployeePropertyList($properties, $orderField, $orderBy, $excludeTerminatedEmployees = false, $excludeConsultant = false) {
        ...
        try {
                ...
                if($excludeConsultant){
                	$q->leftJoin('e.SystemUser user');
                	$q->where('user.user_role_id <> 8 ');
                }
     ...
     * @throws DaoException
     */
    public function getEmployeeCount($includeTerminated = false, $includeConsultant = false) {
    ...
    public function getEmployeeListAsJson($workShift = false) {
        try {
            ...
            foreach ($employeeList as $employee) {
                ...
                if (empty($terminationId) && !isset($employeeUnique[$empNumber])) {
                    $name = trim($employee->getFirstName()) . " " . trim($employee->getLastName());
    ...
    private function _getEmployeeListQuery(&$select, &$query, array &$bindParams, &$orderBy,
        ...
        */
        ...
        $select = 'SELECT e.emp_number AS empNumber, e.employee_id AS employeeId, ' .
                ...
                'e.joined_date as joiningDate, e.emp_marital_status as maritalStatus, e.emp_gender as gender,' .
                'e.emp_middle_name AS middleName, e.termination_id AS terminationId, e.total_experience as totalExperience, ' .
            ...
            foreach ($filters as $searchField=>$searchBy ) {
                if (!empty($searchField) && !empty($searchBy)
                    ...
                    if ($searchField == 'sub_unit') {
                       ...
                       $query.=
                '  LEFT JOIN hs_hr_emp_project eprj ON e.emp_number = eprj.emp_number '.
                '  LEFT JOIN ohrm_project proj ON eprj.project_id = proj.project_id';
                        $conditions[] = $field . ' LIKE ? ';
                        // Replace multiple spaces in string with wildcards
                        $value = preg_replace('!\s+!', '%', $searchBy);
                        $bindParams[] = '%' . $value . '%';
...
//        }
        ...
        if ($returnType == EmployeeSearchParameterHolder::RETURN_TYPE_OBJECT) {
            ...
            if ($result) {
                while ($row = $statement->fetch() ) {
                    ...
                    $employee->setJoinedDate($row['joiningDate']);
                    $employee->setEmpGender($row['gender']);
                    $employee->setEmpMaritalStatus($row['maritalStatus']);
                    $employee->setTotalExperience($row['totalExperience']);
   ...
   /**
     ...
     * @return array
     */
    public function getSearchEmployeeCount(array $filters = null, $countForDirectory = false) {
		...
		if($countForDirectory){
			$this->_getEmployeeListQueryForDirectory($select, $query, $bindParams, $orderBy,
    			null, null, $filters);
		}else{
    ...
    public function searchEmployeesForDirectory(EmployeeSearchParameterHolder $parameterHolder) {
    	...
    	if ($returnType == EmployeeSearchParameterHolder::RETURN_TYPE_OBJECT) {
    		...
    		if ($result) {
    			while ($row = $statement->fetch() ) {
    				...
    				$employee->setBloodGroup(($row['emp_bldgp']));
    				//$employee->setBloodGroup(($row['emp_bldgp']));
    				$employee->setLinkedInUrl(($row['emp_linkedInUrl']));
    				$employee->setProject(($row['project']));
    				$employee->setEmpSkypeId(($row['emp_skype_id']));
    				$employee->setEmpBirthday(($row['emp_birthday']));
    				$employee->setEmployeeSkills($row['skillName']);//for skills
				//$employee->setEmployeeCertifications($row['certificationName']);//for certifications
    				$employee->setEmpZipcode($row['location']);// for location ,
    				
    				if(($row['major']!="") && ($row['level']!="")){
    					$employee->setEmployeeQualification($row['level']." (".$row['major'].")");	
    				}
    ...
    private function _getEmployeeListQueryForDirectory(&$select, &$query, array &$bindParams, &$orderBy,
    		$sortField = null, $sortOrder = null, array $filters = null) {
    	...
    	*/
    	...
    	$select = 'SELECT e.emp_number AS empNumber, e.employee_id AS employeeId, e.emp_work_email,e.emp_mobile,e.emp_skype_id,' .
    			'e.emp_firstname AS firstName, e.emp_lastname AS lastName, e.emp_bldgp, GROUP_CONCAT(DISTINCT(proj.name)) as project, e.emp_linkedInUrl, e.emp_birthday, ' .
    			'e.emp_middle_name AS middleName, e.termination_id AS terminationId, GROUP_CONCAT(os.name) as skillName, GROUP_CONCAT(DISTINCT(loc.name)) as location, ' .
    			'cs.name AS subDivision, cs.id AS subDivisionId, oee.major as major, oe.name as level, ' .
    			'j.job_title AS jobTitle, j.id AS jobTitleId, j.is_deleted AS isDeleted, ' .
    			'es.name AS employeeStatus, es.id AS employeeStatusId, '.
    			'GROUP_CONCAT(s.emp_firstname, \'## \', s.emp_middle_name, \'## \', s.emp_lastname) AS supervisors,'.
    			'GROUP_CONCAT(DISTINCT loc.id, \'##\',loc.name) AS locationIds';
    	...
    	$query =' FROM hs_hr_employee e ' .
    			...
    			'  LEFT JOIN hs_hr_emp_locations l ON l.emp_number = e.emp_number ' .
    			'  LEFT JOIN ohrm_location loc ON l.location_id = loc.id' .
    			'  LEFT JOIN hs_hr_emp_skill hs ON e.emp_number = hs.emp_number' .
    			'  LEFT JOIN ohrm_skill os ON hs.skill_id = os.id' .
    			'  LEFT JOIN ohrm_emp_education oee ON e.emp_number = oee.emp_number' .
    			'  Left JOIN ohrm_education oe ON oee.education_id = oe.id' .
    			'  LEFT JOIN hs_hr_emp_member_detail em ON e.emp_number = em.emp_number' .
    			'  Left JOIN ohrm_membership om ON em.membship_code = om.id' .
    			...
    			if (!empty($searchField) && !empty($searchBy)
    				...
    				if ($searchField == 'sub_unit') {
    					...
    					*/
    					...
    					$conditions[] = ' os.id = ? ';
    					$bindParams[] = $searchBy;
    				} else if ($searchField == 'membership') {
    					$conditions[] = ' om.id = ? ';
    					$bindParams[] = $searchBy;
    				} else if ($searchField == 'supervisorId') {
...
//    	}
    	...
    	//don't show consultant
    	$conditions[] = "( u.user_role_id <> 8 )";
    ...
    public function updateEmployee($employee) {
    	try {
    		...
    		/* Updating employee record */
    		$q = Doctrine_Query :: create()->update('Employee')
    		->set('emp_firstname', '?', $employee->getFirstName())
    		->set('emp_middle_name', '?', $employee->getMiddleName() )
    		->set('emp_lastname', '?', $employee->getLastName())
    		->set('emp_work_email', '?', $employee->getEmpWorkEmail())
    		->set('emp_mobile', '?', $employee->getEmpMobile())
    		->set('permanentAddress', '?', $employee->getPermanentAddress())
    		->set('employee_id', '?', $employee->getEmployeeId())
    		->set('emp_gender', '?', $employee->getEmpGender())
    		->set('emp_marital_status', '?', $employee->getEmpMaritalStatus())
    		->set('street1', '?', $employee->getStreet1())
    		->set('street2', '?', $employee->getStreet2())
    		->set('city', '?', $employee->getCity())
    		->set('country', '?', $employee->getCountry())
    		->set('province', '?', $employee->getProvince())
    		->set('emp_zipcode', '?', $employee->getEmpZipcode())
    		->set('job_title_code', '?', $employee->getJobTitleCode())
    		->set('joined_date', '?', $employee->getJoinedDate())
    		->where('emp_number = ?', $employee->getEmpNumber());
    		$q->execute();
    		$connection->commit();
    		// @codeCoverageIgnoreStart
    	} catch (Exception $e) {
    ...
    public function getTodaysBirthday($excludePicture){
    	try{
    		$q ='SELECT hs_hr_employee.emp_number, hs_hr_employee.emp_firstname,hs_hr_employee.emp_lastname,hs_hr_employee.employee_id ';
    		if(!$excludePicture) {
    			$q.= ' ,hs_hr_emp_picture.epic_picture ';
    		}
			$q .=' FROM hs_hr_employee LEFT JOIN hs_hr_emp_picture ON hs_hr_employee.emp_number = hs_hr_emp_picture.emp_number '; 
			$q .='WHERE hs_hr_employee.termination_id IS NULL AND DAY(hs_hr_employee.emp_birthday) = DAY(CURDATE()) AND MONTH(hs_hr_employee.emp_birthday)= MONTH(CURDATE()) ORDER BY hs_hr_employee.emp_firstname';
    		$pdo = Doctrine_Manager::connection()->getDbh();
    		$res = $pdo->query($q);
    		$todaysBirthday = $res->fetchAll();
    ...
    public function checksEmployeeExists($empid, $email) {
    	try {
    		$q = Doctrine_Query :: create()
    		->select('employeeId')
    		->from('Employee')
    		->where('employeeId = ?', $empid)
    		->andWhere('emp_work_email = ?',$email);
    		return $q->count();
    	} catch (Exception $e) {
    		throw new DaoException($e->getMessage(), $e->getCode(), $e);
    	}
    ...
    public function getDirectSupervisors($empNumber) {
        try {
            $conn = Doctrine_Manager::connection()->getDbh();
            $sql = "SELECT rt.erep_sup_emp_number
                    FROM hs_hr_emp_reportto rt
                    WHERE rt.erep_sub_emp_number = $empNumber
                    AND rt.erep_reporting_mode = 1";
    ...
    public function getEmployeeProjectHistory($empNumber,$projectId) {      
        try {
            $q = Doctrine_Query::create()
                            ->from('EmployeeProjectHistory p')
                            ->where('p.empNumber = '.$empNumber)
                            ->andWhere('p.customerId = '.$projectId)
     ...
     public function getCurrentProjectTeamMember($empNumber,$CurrentCycleObj) {
        $team=array();
             
        if(!empty($CurrentCycleObj)) { 
        $cycle_start_date=$CurrentCycleObj[0]['period_from'];
         $cycle_end_date=$CurrentCycleObj[0]['period_to'];   
        
         if(!empty($cycle_start_date) && !empty($cycle_end_date)) { 
             
            $pdo = Doctrine_Manager::connection()->getDbh(); 
            $queryProj = "SELECT customer_id from hs_hr_emp_project_history";
            $queryProj.= " WHERE  emp_number = ".$empNumber." AND ((DATE_FORMAT(start_date,'%Y-%m-%d') between DATE_FORMAT('".$cycle_start_date."','%Y-%m-%d') AND  DATE_FORMAT('".$cycle_end_date."','%Y-%m-%d') ) ";
            $queryProj.= " OR ((start_date BETWEEN '".$cycle_start_date."' AND '".$cycle_end_date."') ";
            $queryProj.= " OR (end_date BETWEEN '".$cycle_start_date."' AND '".$cycle_end_date."') ";
            $queryProj.= " OR (start_date <= '".$cycle_start_date."' AND ('".$cycle_end_date."' <= end_date OR end_date is NULL )) ))";               
           
            $resProj = $pdo->query($queryProj);       
            $resObjProj=$resProj->fetchAll(PDO::FETCH_ASSOC);
            
            $queryCurrProj = "SELECT customer_id from hs_hr_emp_project";
            $queryCurrProj.= " WHERE  emp_number = ".$empNumber;
            $resCurrProj = $pdo->query($queryCurrProj);       
            $resObjCurrProj=$resCurrProj->fetchAll(PDO::FETCH_ASSOC);
           
            $allProjects=array();
            if(!empty($resObjCurrProj))
            {
                foreach($resObjCurrProj as $key){
                    $allProjects[]=$key['customer_id'];     
                }
            }
            if(!empty($resObjProj))
            {
                foreach($resObjProj as $key){
                    $allProjects[]=$key['customer_id'];     
                }
            }
            
            $querySupportProj = "SELECT customer_id from  ohrm_customer";
            $querySupportProj.= " WHERE support_function=1"  ;
               
            $resSupportProj = $pdo->query($querySupportProj);       
            $resSupportProj=$resSupportProj->fetchAll(PDO::FETCH_ASSOC);
           
            if(!empty($resSupportProj))
            {
                foreach($resSupportProj as $key){
                    $allProjects[]=$key['customer_id'];     
                }
            }
            ...
            if(!empty($allProjects)) {
             foreach($allProjects as $key=>$value){
                $project=$value;           
                $query = "SELECT emp_number from hs_hr_emp_project_history";
                $query.= " WHERE customer_id=".$project." AND emp_number <> ".$empNumber;
                $query.= " AND ((start_date BETWEEN '".$cycle_start_date."' AND '".$cycle_end_date."') ";
                $query.= " OR (end_date BETWEEN '".$cycle_start_date."' AND '".$cycle_end_date."') ";
                $query.= " OR (start_date <= '".$cycle_start_date."' AND ('".$cycle_end_date."' <= end_date OR end_date is NULL )) )";               
                $res = $pdo->query($query);       
        ...
        if(!empty($team)){
            //Added code
            $jsonArray=array(); 
    		//$employees = $this->getCustomerService()->getEmployeesByCustomerId($customerIds);
			$qEmp = Doctrine_Query :: create()
    		->from('Employee emp')
    		->leftJoin('emp.EmployeeProject eprj')
    		->leftJoin('eprj.Customer c')
    		->whereIn('emp.emp_number', $team)
            ->groupBy('emp.emp_number');            
    		$qEmp->andwhere("termination_id IS NULL");
    		$employees = $qEmp->execute();
    		foreach ($employees as $emp) {
    			if($emp->getEmpNumber() != $empNumber) {
    				$name = $emp->getFirstAndLastNames();
    				$jsonArray[$emp->getEmpNumber()] = $name;
    			}
    		}
         return $jsonArray;
        }
