...
class addCandidateAction extends sfAction {
    ...
    public function execute($request) {
        ...
        $flag = $request->getPostParameter('hdnFlag');
        ...
        'isRecruitmentManager' => $userObj->isRecruitmentManager(), 'isConsultant'=> $userObj->isConsultant() , 'flag' => $flag);
        $this->setForm(new AddCandidateForm(array(), $param, true));
        /* Added By : Shagupta Faras
         * Added On: 17-07-2014
         * DESC: Candidate can apply for multiple vacancies
         */    
        $vacancyList=$this->form->getActiveVacancyList();
        $this->vacancyList=$vacancyList;
        if($this->candidateId>0) {
        $candidateVacancyList=$this->form['vacancy']->getValue();     
        }   
        $this->vacancyOptions='';
        if(!empty($vacancyList))
        {
            foreach($vacancyList as $key=>$value){
               if($key=='') {
                   $this->vacancyOptions.= '<option selected="selected" value="'.$key.'">'.$value.'</option>' ;
               } else {
                   if(isset($candidateVacancyList) && in_array($key,$candidateVacancyList))
                   continue;    
                   else        
                   $this->vacancyOptions.= '<option value="'.$key.'">'.$value.'</option>' ;
               }    
            }
        }   
        ...
        if ($this->candidateId > 0) {
            ...
            if (!($userObj->isAdmin() || $userObj->isHiringManager() || $userObj->isRecruitmentManager()||$userObj->isConsultant())) {
                $this->redirect('recruitment/viewCandidates');
            }
        }

        $this->onlyInterviewer=$this->getCandidateService()->isOnlyInterviewer($this->candidateId);
        ...
        if ($request->isMethod('post')) {
            ...
            if (($_FILES['addCandidate']['size']['resume'] > 1024000) || ($_FILES['addCandidate']['error']['resume'] && $_FILES['addCandidate']['name']['resume'])) {
                ...
                $result=$request->getParameter($this->form->getName());
                $this->form->bind($request->getParameter($this->form->getName()), $request->getFiles($this->form->getName()));
                $file = $request->getFiles($this->form->getName());
                if ($this->form->isValid()) {
                /* Added By: Shagupta Faras
                *  Added On: 16-07-2014
                 * DESC: Send multiple vacacancies to save
                */
                $result = $this->form->save($result['vacancy']);
                if (isset($result['messageType'])) {
                    $this->messageType = $result['messageType'];
                    $this->message = $result['message'];
                    $this->invalidFile = true;
                    $this->getUser()->setFlash('addcandidate.'.$result['messageType'], $result['message']);
