...
class LeaveApplicationService extends AbstractLeaveAllocationService {
    ...
    public function applyLeave(LeaveParameterObject $leaveAssignmentData) {
		...
		if($leaveType->getName() == LeaveType::LEAVE_TYPE_WFH ){
			if(!$this->hasWFHBalance($leaveAssignmentData)){
				throw new LeaveAllocationServiceException('Failed to Submit: WORK FROM HOME limit (3/month) Exceeded.');
			}
		}
...
//        }
        ...
        if ($this->isEmployeeAllowedToApply($leaveType)) {
            ...
            if (count($nonHolidayLeaveDays) > 0) {
                ...
                if($leaveAssignmentData->getLeaveApplyBy() == 'cronJob') {
                	$employee = $this->getEmployeeService()->getEmployee($leaveAssignmentData->getEmployeeNumber());
            ...
            //if ($this->isValidLeaveRequest($leaveRequest, $leaves)) {
                try {
                    if($leaveAssignmentData->getLeaveApplyBy() == 'cronJob') { // using support user as cron job
                    	$loggedInUserId = 282;
                    	$loggedInEmpNumber = 285;
                    } else {
                    ...
                    if (!empty($leaveComment)) {                                                       
                        if (!empty($loggedInEmpNumber)) {
                            ...
                            $createdBy = $employee->getFirstAndLastNames();
                        } else {
