...
class LeaveEntitlementDao extends BaseDao {
    ...
    /* Added by Sujata To get Emplyee Joining date */
    public function searchEmployeeJoiningDate($emp) {
        try {
            $q = Doctrine_Query::create()->from('Employee e');
            $params = array();
            if (!empty($emp)) {
                $q->addWhere('e.emp_number = :emp_number');
                $params[':emp_number'] = $emp['empNumber'];
            }
    ...
    public function searchLeaveEntitlements(LeaveEntitlementSearchParameterHolder $searchParameters) {
        try {
            ...
            /* Modified by sujata to get date in mysql date format */
            $fromDate = date('Y-m-d',strtotime($searchParameters->getFromDate()));
            $toDate = date('Y-m-d',strtotime($searchParameters->getToDate()));
    ...
    public function getLeaveBalance($empNumber, $leaveTypeId, $asAtDate, $date = NULL) {
        ...
        if ($result) {
            if ($statement->rowCount() > 0) {
                ...
                if (!empty($dateLimits)) {
                $balance->setCycle(date("D, d M Y",strtotime($dateLimits[0])).' - '.date("D, d M Y",strtotime($dateLimits[1])));
                }
            }
        }
    ...
    public function isDifferentLeavePeriodDates($fromDate,$toDate,$empId)
    {
        $result=Doctrine::getTable('LeaveEntitlement')->createQuery()
                ->select('id')
               ->where('emp_number=?', $empId)
               ->andWhere('? BETWEEN from_date AND to_date', $fromDate)
               ->andWhere('? BETWEEN from_date AND to_date', $toDate)
               ->execute()->toArray();
         if (empty($result))           
                return true;
            else
