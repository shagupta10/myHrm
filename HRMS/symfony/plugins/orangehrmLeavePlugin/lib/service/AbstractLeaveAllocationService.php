...
abstract class AbstractLeaveAllocationService extends BaseService {
    ...
    /**
     * Checks whether user has sufficent Work from home balance (3 WFH in a month).
     */
    public function hasWFHBalance(LeaveParameterObject $leaveAssignmentData){
	    $hasWFHBalance = true;
	    $fromDate = $leaveAssignmentData->getFromDate();
	    $toDate = $leaveAssignmentData->getToDate();
	    
	    if (strlen($fromDate) > 0 && strlen($toDate) > 0) {
		    $fromMonth=date("F",strtotime($fromDate));
		    $fromYear=date("Y",strtotime($fromDate));
		    $toMonth=date("F",strtotime($toDate));
		    $toYear=date("Y",strtotime($toDate));
		    $empNumber = $leaveAssignmentData->getEmployeeNumber();
		    if($fromYear != $toYear){
		    	$hasWFHBalance = $this->_checkWfhBalance($empNumber, $fromDate,null, $leaveAssignmentData);
		    	if($hasWFHBalance){
		    		$hasWFHBalance =  $this->_checkWfhBalance($empNumber, null,$toDate, $leaveAssignmentData);
		    	}
			   ...
			   $hasWFHBalance =  $this->_checkWfhBalance($empNumber, $fromDate,$toDate, $leaveAssignmentData);
		    }
	    }
	    return $hasWFHBalance;
    }
    
    /**
     * Checks whether there is suffient WFH balance or not. If not then return false else return true
     */
    private function _checkWfhBalance($empNumber, $from, $to, LeaveParameterObject $leaveAssignmentData){
	    if(is_null($from)){
	    ...
	    $endDate = date('Y-m-t',strtotime($from));
	    $takenWFH = $this->getLeaveRequestService()->getWFHMonthlyBalance($startDate, $endDate, $empNumber);
	    $days_between = 0;//ceil(abs(strtotime($to) - strtotime($from)) / 86400) + 1;
	    for ($timeStamp = strtotime($from); $timeStamp <= strtotime($to); $timeStamp = $this->incDate($timeStamp)) {
		    $leaveDate = date('Y-m-d', $timeStamp);
		    $workSchedule = $this->getWorkScheduleService()->getWorkSchedule($empNumber);        
		    $isWeekend = $workSchedule->isWeekend($leaveDate, true);
		    $isHoliday = $workSchedule->isHoliday($leaveDate);
		    $isHalfDayHoliday = $workSchedule->isHalfdayHoliday($leaveDate);
		    
            $day = $this->calculateDateDeference($leaveAssignmentData, $isWeekend, $isHoliday, $isHalfday, $isHalfDayHoliday);
            $days_between = $days_between + $day;
	    }
	    
	    if(($days_between + $takenWFH) > LeaveType::WFH_LIMIT_PER_MONTH){
		    return false;
	    }
	    return true;
    }
