...
class ApplyLeaveForm extends sfForm {
    ...
    /* Added by sujata to get employee joined dated from searchEmployeeJoiningDate() */
    public function getJoiningDate() {
        $params['empNumber'] = ltrim($this->getEmployeeNumber(),'0');
        $employeeJoiningDate = $this->getLeaveEntitlementService()->searchEmployeeJoiningDate($params);

        if(count($employeeJoiningDate) > 0){
            $jd = $employeeJoiningDate->getFirst();
            $joiningDate = $jd->getJoinedDate();
        }
        return $joiningDate;
    }
    /* Added by sujata to set employee joined dated */
    public function setJoiningDate($joiningDate) {
        $this->joiningDate = $joiningDate;
    }
    /* Added by sujata to get employee leave entitlement service */
    ...
    /* Added by sujata to set employee leave entitlement service */
    ...
    public function configure() {
		...
		/* Added by sujata set default value to employee joining date and add this field to validator schema */
		$this->setDefault('txtJoiningDate', $this->getJoiningDate());
        $this->validatorSchema['txtJoiningDate'] = new sfValidatorString(array('required' => false));
    ...
    public function postValidation($validator, $values) {
        ...
        if($this->getLeaveEntitlementService()->isDifferentLeavePeriodDates($values['txtFromDate'],$values['txtToDate'],$values['txtEmpID']))
        {
             $errorList['txtToDate'] = new sfValidatorError($validator, ' To Date should be in same leave period of From Date');
        }
        ...
        $minDate=strtotime($period[0]);
        $mayMonth = strtotime(date("Y-m-d",(strtotime(date("Y-m-d", $minDate) . " +1 month"))));
        $today=strtotime(date("Y-m-d"));
        $maxTimeStamp = strtotime($maxDate);
   
        if (is_int($toDateTimeStamp) && ($toDateTimeStamp < $minDate) &&($today >= $mayMonth)) {
            $errorList['txtToDate'] = new sfValidatorError($validator, __('Cannot assign leave for past cycle date '));
        }  
    ...
    protected function getFormWidgets() {
        $widgets = array(
            ...
            /* Addded by sujata */
            'txtJoiningDate' => new sfWidgetFormInputHidden(),
