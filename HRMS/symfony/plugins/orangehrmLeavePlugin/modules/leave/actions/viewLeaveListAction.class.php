...
class viewLeaveListAction extends sfAction {
    ...
    public function execute($request) {        
        ...
        $flag=0;
        $flag=$request->getParameter('onChange');
        if($flag==1)
        	$recordsLimit =  $request->getParameter('recordsPerPage_Limit');
        else
        	$recordsLimit =  $request->getPostParameter('leaveList[recordsPer_Page]');
        ...
        /**
        * Modified by Sujata
		* Add checkbox value for selected direct reportees field is filtered
		*/
        $withDirectReportees = $this->_getFilterValue($values, 'cmbWithDirectReportees', null);     
		...
		/**
        * Modified by Sujata
		* Add direct reportees checkbox value to $searchParams array
		*/
			$searchParams = new ParameterObject(array(
					...
					'withDirectReportees' => $withDirectReportees,
        ...
        $this->setListComponent($list, $recordCount, $page, $NoOfRecords);
    ...
    protected function setListComponent($leaveList, $count, $page, $NoOfRecords) {
