...
class KpiDao extends BaseDao {
	    		...
	    		if (isset($parameters['id']) && $parameters['id'] > 0) {
	    			...
	    			foreach ($parameters as $key => $parameter) {
	    				if (strlen(trim($parameter)) > 0) {
	    					switch ($key) {
	    						case 'jobCode':
	    							$q->andWhere('job_title_code = ?', $parameter);
	    							break;
	    						case 'isDefault':
	    							$q->andWhere('rate_default = ?', $parameter);
	    							break;
	    						default:
	    							break;
	    					}
    ...
    /**
     * Get Job Title for Kpi
     *
     * @param int $jobTitleName
     * @return JobTitle jobTitle
     */
    public function getJobTitleIdForKpi(){
    	try{
    		$jobTitleName = DefineKpi::KPI_JOB_TITLE;
    		$q = Doctrine_Query::create( )
    		->from('JobTitle')
    		->where("jobTitleName = '".$jobTitleName."' AND isDeleted = '0'" );
    		$jobTitleList = $q->fetchOne();
