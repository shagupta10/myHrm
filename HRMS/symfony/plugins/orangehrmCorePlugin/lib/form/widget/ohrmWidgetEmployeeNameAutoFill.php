...
class ohrmWidgetEmployeeNameAutoFill extends sfWidgetFormInput {
    ...
    public function render($name, $value = null, $attributes = array(), $errors = array()) {
        ...
        $empJoiningDateValue         = isset($value['joiningDate'])?$value['joiningDate']:''; // sujata
        ...
        $joiningDateHiddenFieldId  = $this->getJoiningDateHiddenFieldId($name);
                ...
                if( loadingMethod != 'ajax'){
                    ...
                    nameField.autocomplete(employees_%s, {
                      ...
                      ,matchContains:true
                            ...
                            joiningDateIdStoreField.val(item.joiningDate);
                        }
                        ...
                        $.ajax({
                               ...
                               success: function(employeeList){
                                     ...
                                     nameField.autocomplete(employeeList, {
                                                ...
                                                joiningDateIdStoreField.val(item.joiningDate);
                                                /* Added by sujata */
                                                if($('#entitlements_leave_type').length && $('#entitlements_entitlement').length){
                                                	setTimeout(function () {
                                                    	if($( "#entitlements_leave_type" ).val() != ''){
...
EOF
        ...
        return "\n\n" . $html . "\n\n" . $this->getHiddenFieldHtml($name, $empIdValue) . "\n\n" . $this->getJoiningDateHiddenFieldHtml($name, $empJoiningDateValue). "\n\n" . $javaScript . "\n\n";
    ...
    protected function getJoiningDateHiddenFieldHtml($name, $value) {
        
        //$hiddenName = substr($name, 0, strlen($name) - 1) . '_id]';
        $hiddenName = $name . '[joiningDate]';
        $hiddenId   = $this->getJoiningDateHiddenFieldId($name);
	...
	protected function getJoiningDateHiddenFieldId($name){
		return $this->generateId($name) . '_joiningDate';
	}
    ...
    protected function getEmployeeListAsJson($employeeList) {
        ...
        foreach ($employeeList as $employee) {
            ...
            $jsonArray[$employee['empNumber']] = array('name' => $name, 'id' => $employee['empNumber'], 'joiningDate' => $employee['joined_date']);
        }
