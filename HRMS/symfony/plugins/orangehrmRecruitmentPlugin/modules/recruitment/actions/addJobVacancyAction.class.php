    ...
    public function execute($request) {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($this->form->isValid()) {
            	$currentVacancy = $this->getVacancyService()->getVacancyById($this->vacancyId);
            	$changedvacancy = $this->form->getVacancyValues();
            	if(($currentVacancy['status'] == 1) && ($changedvacancy->getStatus() == 0)) {
            		$count = $this->getVacancyService()->getActiveCandidateCountByVacancyId($this->vacancyId);
            		if($count == 0) {
            			$this->vacancyId = $this->form->save();
            			$this->getUser()->setFlash('success', __(TopLevelMessages::SAVE_SUCCESS));
            		} else {
            			$this->getUser()->setFlash('error.nofade', __('Unable to close Vacancy. Some candidates are in active status. Please Reject/Hire candidates to close vacancy'));
            		}
            	} else {
            		$this->vacancyId = $this->form->save();
            		$this->getUser()->setFlash('success', __(TopLevelMessages::SAVE_SUCCESS));
