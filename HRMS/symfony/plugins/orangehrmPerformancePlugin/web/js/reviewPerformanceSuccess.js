$(document).ready(function(){
    ...
    $( "input[name^=txtSelfRate]" ).keyup(function() {
		calculateSelfKPIRatings();
	});
	$( "input.smallInput").keyup(function() {
		checkSubmit();
	});
	...
	$( "input.ratingInput" ).keyup(function() {
		calculateFinalRatings();
	});

});

function saveGoals(reviewerID){
	var goalText = $('#editGoal_'+reviewerID).val()
	$.ajax({
		type: 'GET',
		...
		success: function(data) {
			if(data != 0) {
				$('#editGoal_'+reviewerID).val(data);
				$('#goalText_'+reviewerID).html(data);
			}
			$('#editGoalContainer_'+reviewerID).hide();
			$('div#editButton_'+reviewerID).show();
			$('#goals_'+reviewerID).show();
		}
	});
}  
function showEditGoals(reviewerID){
	$('#editGoalContainer_'+reviewerID).show();
	$('div#editButton_'+reviewerID).hide();
	$('#goals_'+reviewerID).hide();
}
            ...
            if( !isNaN(max) || !isNaN(min)){
                ...
                if(this.value != '' && isNaN(rate) ){
                    ...
                    if( (rate > max) || (rate < min) ){
                        valid = false;                        
                        $(this).addClass('validation-field');
                        $(this).next('span.validation-error').text(rangeError);
                    }
                }
            }
        });
...
function calculateKPIRatings(){
	...
	var KPIfinalRatings = 0;
	if(ratings != 0 && noOfKpis != 0){
		KPIfinalRatings = ratings/noOfKpis;
	}
	$('#txtKpiRating').val(new Number(KPIfinalRatings).toFixed(2));
}

function calculateSelfKPIRatings(){
	var ratings = 0;
	var noOfKpis = 0;
	$('input[name^=txtSelfRate]').each(function(){
		if( 'N/A' != $.trim(this.value) && 'n/a' != $.trim(this.value) && 'NA' != $.trim(this.value) && 'na' != $.trim(this.value)){
			noOfKpis++;
			if ($.trim(this.value) != '') {
				ratings+= parseFloat(this.value);
	...
	var selfKPIfinalRatings = 0;
	if(ratings != 0 && noOfKpis != 0){
		selfKPIfinalRatings = ratings/noOfKpis;
	}
	$('#txtSelfKpiRating').val(new Number(selfKPIfinalRatings).toFixed(2));	
}
...
function calculateFinalRatings(){
	if(checkSubmit()){
		var ratings = 0;
    	var noOfKpis = 0;
    	$('input.ratingInput').each(function(){
			if( 'N/A' != $.trim(this.value) && 'n/a' != $.trim(this.value) && 'NA' != $.trim(this.value) && 'na' != $.trim(this.value)){
				noOfKpis++;
				if ($.trim(this.value) != '') {
					ratings+= parseFloat(this.value);
    	...
    	$('#txtfinalRating').val(new Number(finalRatings).toFixed(2));
		$('#finalRating').val(new Number(finalRatings).toFixed(2));
	}
}
...
function isCommented() {
    var valid=true;
    $("textarea.selfComments").each(function() {
