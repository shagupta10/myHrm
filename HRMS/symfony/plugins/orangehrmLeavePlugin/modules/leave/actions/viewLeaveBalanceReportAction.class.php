...
class viewLeaveBalanceReportAction extends sfAction {
    ...
    public function execute($request) {
        ...
        if ($runReport) {
            ...
            $No_of_Records =  $request->getPostParameter('leave_balance[LBR_recordsPer_Page_Limit]');
            $this->recordsLimit = $No_of_Records;
            $reportBuilder = new ReportBuilder();
            $numOfRecords = $reportBuilder->getNumOfRecords($reportId, $values);
            if($No_of_Records){
            	$maxPageLimit = $noOfRecords = $No_of_Records;
            }else{
            	$maxPageLimit = $noOfRecords = 10; //sfConfig::get('app_items_per_page'); //$reportBuilder->getMaxPageLimit($reportId);
