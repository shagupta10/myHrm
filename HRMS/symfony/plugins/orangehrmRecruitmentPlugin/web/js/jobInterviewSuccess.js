...
$(document).ready(function() {
    ...
    $(".formInputInterviewer").autocomplete(employees, {
    	...
    	$('#'+currentElementId).attr('interviewerId', item.id);
    ...
    $('.removeText').live('click', function(){
        ...
        $('#interviewer_'+result).removeAttr('interviewerId');
        validateInterviewerNames();
...
function checkIfInterviewerAvailable(){
    var data;
    interviewersArray = [];
    var numInterviewers = 5;
    
    for(var i=1; i<=numInterviewers; i++){
    	var interviewerId = $('#interviewer_'+i).attr('interviewerId');
    	if( typeof interviewerId === "undefined"){
    	}else{
    		interviewersArray.push($('#interviewer_'+i).attr('interviewerId'));
    	}
    }
    var flag = false;
    var date = $("#jobInterview_date").val();
    var time = $("#jobInterview_time").val();
    var intTitle = $("#jobInterview_name").val();
    if(date && time && interviewersArray.length>0 && intTitle){
		$.ajax({
			 type: "POST",
			 async:false,
			 data: {interviewersArray:interviewersArray,date:date,time:time},
		     url: isInterviewerAvailable,
		     dataType: 'json',
		     success: function(data) {
		    	 if(data.success==0){
		    		 
		    		 $('#interviewer_unavailable_list').html("");
		    		 $('#interviewer_unavailable_blk').show();
		    		 $.each( data.unavailable_interviewers, function( key, value ) {
			    		  	var empFirstName = "";
			    		  	var empMiddleName = "";
			    		  	var empLastName = "";
			    	 		if(value.emp_firstname!=''){
			    	 			empFirstName = value.emp_firstname;
			    	 		}
			    	 		if(value.emp_middle_name!=''){
			    	 			empMiddleName = value.emp_middle_name;
			    	 		}
			    	 		if(value.emp_lastname!=''){
			    	 			empLastName = value.emp_lastname;
			    	 		}
			    	 		
			    		  $("#interviewer_unavailable_list").append("<li>"+ empFirstName + " "+empMiddleName+" "+empLastName+"</li>");
			    		  flag =  false;
			    	});
		    	 }else{
		    		 $('#interviewer_unavailable_blk').hide();
		    		 flag = true;
		    	 }
		     }
		});
