...
class ViewCandidateActionForm extends BaseForm {
    ...
    public function configure() {
        ...
        if ($this->candidateId > 0) {
             ...
             * DESC : Get Status widget for multiple vacancies
             */
            $this->candidate = $this->getCandidateService()->getCandidateById($this->candidateId);
            //$obj=Doctrine::getTable('JobCandidateVacancy')->findByCandidateId($this->candidateId);
            $obj=$this->getCandidateService()->getCandidateVacancy($this->candidateId);
            //$candidateVacancy = $this->candidate->getJobCandidateVacancy();
            foreach($obj as $candidateVacancy){
            if ($candidateVacancy->getVacancyId() > 0) {
                $userObj = new User();
                ...
                $userRoleArray = array();
                    ...
                    if ($this->isRecruitmentManager) {
                        $userRoleArray['isRecruitmentManager'] = true;
                    }
