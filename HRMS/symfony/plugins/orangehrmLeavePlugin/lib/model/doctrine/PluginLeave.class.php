...
abstract class PluginLeave extends BaseLeave {
    ...
    private static $nonCompOffStatuses = array(
    	self::LEAVE_STATUS_LEAVE_WEEKEND,
        self::LEAVE_STATUS_LEAVE_HOLIDAY,
        self::LEAVE_STATUS_LEAVE_TAKEN
    );
    ...
    /**
     * get the comoff leave list status which need to add/approve compoff
     */
    public static function getCompOffStatusTextList(){
	    $statusList = self::getLeaveStatusListFromDb();
	    $compOffStatus = array();
	    
	    // filter out leave taken and holidays
	    foreach ($statusList as $key => $status) {
		    if (!in_array($key, self::$nonCompOffStatuses)) {
			    $compOffStatus[$key] = $status;	
		    }
	    }
	    $leaveStatuses = array_map('strtolower', $compOffStatus);
