...
class LeaveListForm extends sfForm {
    ...
    public function configure() {
        ...
        $widgets['recordsPer_Page'] = new sfWidgetFormInputHidden(array(), array());
        // Set default from/to to current leave period.
        $leavePeriod = $this->getLeavePeriodService()->getCurrentAndFutureLeavePeriodByDate(date('Y-m-d', time()));
        $defaults['calFromDate'] = set_datepicker_date_format($leavePeriod[0]);
        $defaults['calToDate'] = set_datepicker_date_format($leavePeriod[1]);
        ...
        $validators['calToDate'] = new ohrmDateValidator(       
                array('date_format' => $inputDatePattern, 'required' => false),
                array('invalid'=>'Date format should be'. $inputDatePattern)); 
        $validators['recordsPer_Page'] = new sfValidatorString(array('required' => false));
 ...
 */            
            /**
	        * Modified by Sujata
			* TODO check cmbWithDirectReportees if searching with Direct Reportees
			*/            
            if (isset($_SESSION['isAdmin']) && $_SESSION['isAdmin'] == 'Yes') {
            $widgets['cmbWithDirectReportees'] = new sfWidgetFormInputCheckbox(array('value_attribute_value' => 'on'));
	        $labels['cmbWithDirectReportees'] =  __('Direct Reportees');
	        $validators['cmbWithDirectReportees'] =  new sfValidatorBoolean(array('true_values' => array('on'), 'required' => false)); 
			$defaults['cmbWithDirectReportees'] = 'on';
