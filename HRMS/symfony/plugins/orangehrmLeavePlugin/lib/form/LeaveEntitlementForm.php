	...
	protected function setLeaveTypeWidget() {
		...
		if (count($leaveTypeList) == 0) {
			...
			foreach ($leaveTypeList as $leaveType) {
				if(strtolower(LeaveType::LEAVE_TYPE_COFF) != strtolower($leaveType->getName()))
				{
					if (is_null($defaultLeaveTypeId)) {
						$defaultLeaveTypeId = $leaveType->getId();
					}
					$choices[$leaveType->getId()] = $leaveType->getName()." [".$leaveType->getDaysLimit()." Days]";
					$entitlementDaysMap[$leaveType->getId()]= $leaveType->getDaysLimit();
				}           
			}
		}
		/* Added by sujata to get first item in dropdown to be empty */
		...
		$this->setEntitlementDaysMap($entitlementDaysMap);
	}
	
	public function setEntitlementDaysMap($entitlementDaysMap){
		$this->entitlementDaysMap = $entitlementDaysMap;
	}
	
	public function getEntitlementDaysMap()
	{
		return Json_encode($this->entitlementDaysMap);
	}
