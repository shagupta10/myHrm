...
<div id="addPaneAttachments">
    ...
    <div class="inner">
        <form name="frmPerformanceAttachment" id="frmPerformanceAttachment" method="post" enctype="multipart/form-data" action="<?php echo url_for('performance/updateAttachment'); ?>">
            <?php echo $form['_csrf_token']; ?>
            <?php echo $form["reviewId"]->render(); ?>
            <?php echo $form["attachId"]->render(); ?>
...
<div id="attachmentList" class="miniList">
    ...
    <div class="inner">
        <?php include_partial('global/flash_messages', array('prefix' => 'performanceAttachmentPane')); ?>
        <form name="frmRecDelAttachments" id="frmRecDelAttachments" method="post" action="<?php echo url_for('performance/deleteAttachments'); ?>">
                        ...
                        foreach ($attachmentList as $attachment) {
                            ...
                            <tr class="<?php echo $cssClass; ?>">
                                ...
                                <td><a title="<?php echo $attachment->fileName; ?>" target="_blank" class="fileLink"
                                       href="<?php echo url_for('performance/viewAttachment?attachId=' . $attachment->id); ?>"><?php echo $attachment->fileName; ?></a></td>
                                <td><?php echo add_si_unit($attachment->fileSize); ?></td>
                                <td><?php echo $attachment->fileType; ?></td>
                                <td class="comments">
                                    <?php echo $attachment->comment; ?>
                                </td>
                                <td><?php echo $attachment->getEmployee()->getFirstAndLastNames(); ?></td>
                                <td>
                                	<?php if($showEdit){ ?>
...
$(document).ready(function() {
    ...
    $("#frmPerformanceAttachment").data('add_mode', true);

    jQuery.validator.addMethod("attachment",
        function() {

            var addMode = $("#frmPerformanceAttachment").data('add_mode');
            if (!addMode) {
                return true;
            } else {
                var file = $('#performanceAttachment_ufile').val();
                return file != "";
            }
        }, ""
        );
    var attachmentValidator =
    $("#frmPerformanceAttachment").validate({

        rules: {
            'performanceAttachment[ufile]' : {
                attachment:true
            },
            'performanceAttachment[comment]': {
                maxlength: 250
            }
        },
        messages: {
            'performanceAttachment[ufile]': lang_PleaseSelectAFile,
            'performanceAttachment[comment]': {
    ...
    $('#attachmentList a.editLink').click(function(event) {
        ...
        $('#performanceAttachment_attachId').val(seqNo);
        $('#performanceAttachment_ufile').removeAttr("disabled");
            
        $('#performanceAttachment_comment').val(description);

        $("#frmPerformanceAttachment").data('add_mode', false);
    ...
    $('#btnAddAttachment').click(function() {
        ...
        $('#performanceAttachment_reviewId').val('');
        $('#attachmentEditNote').text('');
        $('#performanceAttachment_comment').val('');
    ...
    $('#cancelButton').click(function() {
        ...
        $('#performanceAttachment_ufile').val('');
        $('#performanceAttachment_comment').val('');
        $('#performanceAttachment_attachId').val('');
        $('#attachmentList').show();
    ...
    $('#btnSaveAttachment').click(function() {
        $('#performanceAttachment_reviewId').val(id);
        $("#frmPerformanceAttachment").data('add_mode', true);
        $('#frmPerformanceAttachment').submit();
    });
        
    $('#btnCommentOnly').click(function() {
        $('#performanceAttachment_commentOnly').val('1');
        $("#frmPerformanceAttachment").data('add_mode', false);
        $('#frmPerformanceAttachment').submit();
    });
