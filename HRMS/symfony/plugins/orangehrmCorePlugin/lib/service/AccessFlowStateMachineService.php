    ...
    public function getAllAlowedRecruitmentApplicationStates($flow, $role) {
        ...
        if (is_null($result)) {
            ...
            foreach ($result as $rslt) {
	            if($rslt->getState() == "HOLD1" || $rslt->getState() == "HOLD2" || $rslt->getState() == "HOLD3"){
		            $state = "HOLD";
	            }else{
	            	$state = $rslt->getState();
	            }
	            
	            if($rslt->getResultingState() == "HOLD1" || $rslt->getResultingState() == "HOLD2" || $rslt->getResultingState() == "HOLD3"){
		            $resultingState = "HOLD";
	            }else{
	            	$resultingState = $rslt->getResultingState();
	            }
	            
                $stateList[] = $state;
                $resultingStateList[] = $resultingState;
            }
