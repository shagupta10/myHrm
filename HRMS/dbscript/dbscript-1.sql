...
create table `hs_hr_emp_dependents` (
  ...
  `ed_relationship_type` ENUM('child', 'other', 'mother','spouse', 'father',  'father-in-law',  'mother-in-law' ),
...
create table `ohrm_customer` (
  ...
  `support_function` tinyint(1) default 0,
...
CREATE TABLE `hs_hr_performance_review_reviewer` (
  ...
  CONSTRAINT `fk_review` FOREIGN KEY (`review_id`) REFERENCES `hs_hr_performance_review` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
create table `hs_hr_performance_period` (
  `id` int(7) NOT NULL AUTO_INCREMENT,
  `period_from` date DEFAULT NULL,
  `period_to` date DEFAULT NULL,
  `due_date` date DEFAULT NULL,
  `created_date` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `one_on_one_review_date` date DEFAULT NULL,
...
create table `ohrm_job_vacancy`(
    ...
    `consultants` varchar(200) default null,
	primary key (`id`)
)engine=innodb default charset=utf8;
...
create table `ohrm_job_vacancy_hiring_manager` (
	 `id` int(11) NOT NULL AUTO_INCREMENT,
	 `vacancy_id` int(11) DEFAULT NULL,
	 `hiring_manager_id` int(11) DEFAULT NULL,
	 PRIMARY KEY (`id`),
	 KEY `vacancy_id` (`vacancy_id`),
	 KEY `hiring_manager_id_2` (`hiring_manager_id`),
	 CONSTRAINT `ohrm_job_vacancy_hiring_manager_ibfk_1` FOREIGN KEY (`hiring_manager_id`) REFERENCES `hs_hr_employee` (`emp_number`),
	 CONSTRAINT `ohrm_job_vacancy_hiring_manager_ibfk_2` FOREIGN KEY (`vacancy_id`) REFERENCES `ohrm_job_vacancy` (`id`)
) engine=innodb default charset=utf8;
...
CREATE TABLE `ohrm_leave_comp_off` (
  `id` INT(10) NOT NULL AUTO_INCREMENT,
  `emp_number` INT(7) NOT NULL,
  `number_of_days` INT(2) NOT NULL,
  `compOff_details` VARCHAR(256) DEFAULT NULL,
  `status` SMALLINT(6) DEFAULT NULL,
  `created_date` DATE DEFAULT NULL,
  `updated_date` DATE DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `compOff_emp_leave_Fk1` (`emp_number`),
  CONSTRAINT `compOff_emp_leave_Fk1` FOREIGN KEY (`emp_number`) REFERENCES `hs_hr_employee` (`emp_number`)
) ENGINE=INNODB DEFAULT CHARSET=utf8;
...
CREATE TABLE IF NOT EXISTS `ohrm_user_reset_password` (
  `id` int(10) NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `hs_hr_emp_project` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `emp_number` int(11) NOT NULL,
  `customer_id` int(11) NOT NULL,
  `project_id` int(11) NOT NULL,
...
CREATE TABLE `ohrm_job_candidate_requests` (
  `request_id` int(13) NOT NULL AUTO_INCREMENT,
  `candidate_id` int(13) DEFAULT NULL,
  `old_vacancy_id` int(13) DEFAULT NULL,
  `new_vacancy_id` int(13) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `hs_hr_emp_360_feedback` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `emp_number` int(7) DEFAULT NULL,
  `reviewers_number` int(7) DEFAULT NULL,
  `positive_feedback` text,
  `negetive_feedback` text,
...
CREATE TABLE `hs_hr_performance_attachment` (
  `id` INT(13) NOT NULL AUTO_INCREMENT,
  `review_id` INT(13) NOT NULL,
  `file_name` VARCHAR(200) NOT NULL,
  `file_type` VARCHAR(200) DEFAULT NULL,
  ...
  CONSTRAINT `hs_hr_per_attachment_ibfk_1` FOREIGN KEY (`review_id`) REFERENCES `hs_hr_performance_review` (`id`) ON DELETE CASCADE
) ENGINE=INNODB DEFAULT CHARSET=utf8;

CREATE TABLE `ohrm_api_authentication` (
...
CREATE TABLE IF NOT EXISTS `hs_hr_emp_certification` (
  `certification_id` int(11) NOT NULL AUTO_INCREMENT,
  `emp_number` int(11) NOT NULL,
  `name` varchar(100) NOT NULL,
  `Institute` varchar(100) NOT NULL,
  `cattach_name` varchar(100) NOT NULL,
  `cattach_type` varchar(100) NOT NULL,
  `cattach_length` int(100) NOT NULL,
  `cattach` mediumblob NOT NULL,
...
CREATE TABLE `ohrm_interview_feedback`(
  `id` INT(13) NOT NULL AUTO_INCREMENT,
  `history_id` INT(13) NOT NULL,
  `soft_skill` VARCHAR(20) NULL,
  `attitude` VARCHAR(20) NULL,
  `communication` VARCHAR(20) NULL,
  `comments` VARCHAR(20) NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8;

CREATE TABLE `ohrm_training` (
  `id` int(7) NOT NULL AUTO_INCREMENT,
  ...
  `attendee_point` int(2) DEFAULT NULL,
  `trainer_point` int(2) DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

CREATE TABLE `ohrm_trainer` (
  `id` int(7) NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `ohrm_training_schedule` (
  `id` int(7) NOT NULL AUTO_INCREMENT,
  `training_id` int(7) DEFAULT NULL,
...
CREATE TABLE `ohrm_training_trainer` (
  `id` int(7) NOT NULL AUTO_INCREMENT,
  `trainer_type` int(1) DEFAULT NULL,
  `training_id` int(7) DEFAULT NULL,
  `emp_number` int(7) DEFAULT NULL,
  `trainer_id` int(7) DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

CREATE TABLE `ohrm_training_attendees` (
  `id` int(7) NOT NULL AUTO_INCREMENT,
  `emp_number` int(7) DEFAULT NULL,
  `training_id` int(7) DEFAULT NULL,
...
CREATE TABLE `ohrm_training_attendance` (
  `id` int(7) NOT NULL AUTO_INCREMENT,
  `schedule_id` int(7) DEFAULT NULL,
  `emp_number` int(7) DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

CREATE TABLE `ohrm_training_points` (
  `id` int(7) NOT NULL AUTO_INCREMENT,
  `emp_number` int(7) DEFAULT NULL,
  `training_id` int(7) DEFAULT NULL,
  `points` int(2) DEFAULT NULL,
  `action` int(1) DEFAULT NULL,
  `added_by` int(7) DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

ALTER TABLE  hs_hr_performance_review 
ADD `previous_objective` TEXT,
ADD `major_accomplishments` TEXT,
ADD `strong_points` TEXT,
ADD `noticed_improvements` TEXT,
ADD `scope_for_improvement` TEXT,
ADD `employee_feedback` TEXT,
ADD `customer_Id` INT(13),
ADD `is_deleted` INT(1) DEFAULT 0,
ADD `feedback_on_accomplishments` TEXT,
ADD `reject_comments` TEXT;

ALTER TABLE hs_hr_performance_review_reviewer
ADD `is_primary` INT(1) DEFAULT 0,
ADD `is_deleted` INT(1) DEFAULT 0;
...
ALTER TABLE  `ohrm_job_candidate` ADD  `current_ctc` DOUBLE NULL ,
ADD  `expected_ctc` DOUBLE NULL ,
ADD  `notice_period` TINYINT NULL ;

ALTER TABLE  `ohrm_job_candidate` ADD  `expected_doj` DATE NULL ,
ADD  `original_location` VARCHAR( 200 ) NULL ,
ADD  `visa_status` INT NULL ;

ALTER TABLE  `ohrm_job_candidate` CHANGE  `visa_status`  `visa_status` VARCHAR( 100 ) NULL DEFAULT NULL;

ALTER TABLE  `hs_hr_employee` ADD  `emp_linkedInUrl` VARCHAR(250) NULL;

ALTER TABLE  `ohrm_job_candidate` ADD `microresume` text,
ADD `preferred_location` varchar(200) ,
ADD `education_detail_degree` varchar(100),
ADD `education_detail_perc` DOUBLE NULL,
ADD `education_detail_spec` varchar(100),
ADD `total_experience` DOUBLE NULL,
ADD `relevant_experience` DOUBLE NULL,
ADD `current_company` varchar(50),
ADD `designation` varchar(50),
ADD `stability` varchar(10) ,
ADD `com_skills` varchar(200),
ADD `key_skills` varchar(200),
ADD `project_details` text,
ADD `employment_type` varchar(15),
ADD `work_gap` DOUBLE NULL,
ADD `edu_gap` DOUBLE NULL,
ADD `alternate_email` varchar(100),
ADD `alternate_number` varchar(30);

ALTER TABLE `ohrm_job_vacancy` ADD (flag_for_vacancy integer(4) default 1);

ALTER TABLE `ohrm_job_vacancy` ADD `experience` VARCHAR(30) NOT NULL , ADD `keySkills` TEXT NOT NULL , ADD `goodToHaveSkills` TEXT NOT NULL;

CREATE TABLE IF NOT EXISTS `hs_hr_emp_project_history` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `emp_number` int(11) NOT NULL,
  `customer_id` int(11) NOT NULL,
  `start_date` date DEFAULT NULL,
...
ALTER TABLE  `hs_hr_emp_project_history` MODIFY COLUMN `customer_id` int(11)  NULL;
ALTER TABLE `hs_hr_emp_project_history` ADD `project_id` INT( 11 ) NOT NULL AFTER `customer_id` ;
ALTER TABLE `ohrm_leave_type` ADD `days_limit` INT(2) NULL DEFAULT NULL ;
