...
class applyLeaveAction extends baseLeaveAction {
    ...
    public function execute($request) {
        ...
        $this->period = $this->getLeavePeriodService()->getCurrentLeavePeriodByDate($todayNextYear->format('Y-m-d'));
        $this->startMonthPeriod=date("m",strtotime($this->period[0]));
        ...
        if ($request->isMethod('post')) {
            ...
            if ($this->applyLeaveForm->isValid()) {
                try {
                    ...
                    $this->getUser()->setFlash('warning.nofade', __($e->getMessage()));
                }
            }
        }
        ...
        $this->whfLimitPerMonth = $wfhLimitDB;
    }
