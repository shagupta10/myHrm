...
class updateAttachmentAction extends sfAction {
    ...
    public function execute($request) {
        ...
        $this->form = new PerformanceAttachmentForm(array(), null, true);

        if ($this->getRequest()->isMethod('post')) {
            if ($_FILES['performanceAttachment']['size']['ufile'] > 1024000 || $_FILES == null) {
...
//                $this->getUser()->setFlash('attachmentMessage', array('success', __(TopLevelMessages::SAVE_SUCCESS)));
                $this->getUser()->setFlash('performanceAttachmentPane.success', __(TopLevelMessages::SAVE_SUCCESS));
            }
        }
        $this->redirect($this->getRequest()->getReferer() . '#attachments');
    }
