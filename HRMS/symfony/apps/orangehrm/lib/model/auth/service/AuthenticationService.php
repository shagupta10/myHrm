...
class AuthenticationService extends BaseService {
    ...
    public function setCredentials($username, $password, $additionalData) {
        $user = $this->getAuthenticationDao()->getCredentials($username, $password);
    ...
    protected function setRoleBasedUserAttributes(SystemUser $user) {
        ...
        if ($user->getIsAdmin() == 'No') {
            ...
            $isConsultant = $user->getIsConsultant();
            $isRecruitmentManager = $user->getIsRecruitmentManager();
        }
        ...
        $sfUser->setAttribute('auth.isConsultant', $isConsultant);
        $sfUser->setAttribute('auth.isRecruitmentManager', $isRecruitmentManager);
    }
    ...
    protected function setRoleBasedUserAttributesToSession(SystemUser $user) {
        ...
        $_SESSION['isConsultant'] = ($isNotAdmin) ? $user->getIsConsultant() : false;
        $_SESSION['isRecruitmentManager'] = ($isNotAdmin) ? $user->getIsRecruitmentManager() : false;
    }
    ...
    public function checkCredentialsForApi($username, $password) {
    	$user = $this->getAuthenticationDao()->getCredentials($username, $password);
    	if (is_null($user) || !$user) {
    		return false;
    	} else {
