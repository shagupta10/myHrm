...
class sfForm implements ArrayAccess, Iterator, Countable
{
  ...
  public function isValid()
  {
	...
	foreach( $this->getFormFieldSchema( ) as $name => $formField )
	{
	   if( $formField->getError( ) != "" )
	   {
	   		if(empty($logString)) {
	   			$logString .= "Symfony form validation has failed in ".get_class($this)."\n";
	   		}
	   		$logString.=" field - ".$name."; error - ".$formField->getError()."\n------\n";
	   }
	}
	if(!empty($logString)) {
		$logger = Logger::getLogger('sfForm');
