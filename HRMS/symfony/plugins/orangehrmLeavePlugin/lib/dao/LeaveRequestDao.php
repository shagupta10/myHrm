...
class LeaveRequestDao extends BaseDao {
    ...
    public function getWFHMonthlyBalance($leaveStartDate, $leaveEndDate ,$employeeId,$leaveTypeId){
        ...
        $q = Doctrine_Query::create()
                ->select('SUM(length_days) as total_days')
                ->from('Leave')
                ->where("emp_number =?", $employeeId)
                ->andWhere("leave_type_id =?", $leaveTypeId)
                ->andWhereNotIn("status ", $leaveStatusNotConsider)
                ->andWhere("date BETWEEN ? AND ?", array($leaveStartDate, $leaveEndDate));
        $numberOfDays = $q->fetchOne();

        return $numberOfDays->getTotalDays();
    }
    ...
    public function searchLeaveRequests($searchParameters, $page = 1, $isCSVPDFExport = false, $isMyLeaveList = false,
            $prefetchLeave = false, $prefetchComments = false, $isDirectReportees = true) {
        $this->_markApprovedLeaveAsTaken();
        /**
        * Modified by Sujata
		* Update $isDirectReportees value according to checkbox set on the form
		*/
		$isDirectReportees = $searchParameters->getParameter('withDirectReportees'); 
        ...
        if ($isDirectReportees) {
            $q->leftJoin('lr.ReportTo rt');
            $q->leftJoin('rt.ReportingMethod rm');
        }
        ...
        $sortField = $searchParameters->getParameter('sortField');
        $sortOrder = $searchParameters->getParameter('sortOrder');
...
//        }
		...
		/**
        * Modified by Sujata
		* Change query to add field for direct reportees, i.e. supervisor in Table hs_hr_emp_reportto
		*/
        if ($isDirectReportees) {
        	$q->andWhere("rt.supervisorId = ?", sfContext::getInstance()->getUser()->getEmployeeNumber()); 
            $q->andWhere("rm.id = 1");
...
//        $q->orderBy('l.date DESC, em.emp_lastname ASC, em.emp_firstname ASC');
        ...
        if(!empty($sortField)){
        	if($sortField == 'emp_firstname'){
        		$q->orderBy('em.'.$sortField.' '.$sortOrder);
        	}
        	if($sortField == 'date'){
        		$q->orderBy('l.'.$sortField.' '.$sortOrder);
        	}
        	if($sortField == 'leave_type_id'){
        		$q->orderBy('lr.'.$sortField.' '.$sortOrder);
        	}
        	if($sortField == 'numberOfDays'){
        		$q->orderBy('l.length_days'.' '.$sortOrder);
        	}
...
//        }
    ...
    public function getLeaveByDateAndEmpNumber($empNumber, $date, $isHalfLeave=false) {
    	try {
    		$q = Doctrine_Query::create()
	    	    ->from('Leave')
	    	    ->where('emp_number = ?', $empNumber)
	    	    ->andWhereIn('status', array(Leave::LEAVE_STATUS_LEAVE_PENDING_APPROVAL, 
	    	    	Leave::LEAVE_STATUS_LEAVE_TAKEN,
	    	    	Leave::LEAVE_STATUS_LEAVE_APPROVED));
	    	if(is_array($date)) {
	    		$q->andWhere('date >= ?', $date['from']);
			    $q->andWhere('date <= ?', $date['to']);
	    	} else {
	    		$q->andWhere('date = ?', $date);
	    	}
	    	if($isHalfLeave) {
				$q->andWhere('lenght_hours = ?',Leave::LEAVE_HALF_DAY_TIME);	
	    	}
	    	return $q->execute();
    	} catch (Exception $e) {
    		throw new DaoException($e->getMessage());
