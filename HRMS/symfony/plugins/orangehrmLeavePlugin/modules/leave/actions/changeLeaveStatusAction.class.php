...
class changeLeaveStatusAction extends baseCoreLeaveAction {
	...
	public function execute($request) {
		$leaveCompOffs = $request->getParameter('leaveCompOff');
		
		if ($request->isMethod('post')) {
			if(empty($leaveCompOffs)){
				...
				$changes = $request->getParameter('leaveCompOff');
				$actions = $request->getParameter('select_compoff_leave_action');
				$changedActions = array();
				//this is to bypass the approval/rejection comment
				foreach ($changes as $k => $v) {
					if (trim($v) != "" && $actions[$v] != "") {
						$changedActions[$k] = $actions[$k];
					}
				}

//				   $logger = Logger::getLogger('actions.changeLeaveStatusAction');
//					foreach ($changedActions as $k => $v) {
//					$logger->error('actions.changeLeaveStatusAction value:  '. $v);
//					$logger->error('actions.changeLeaveStatusAction: key '. $k);
//					if (trim($v) != "") {
//						$logger->error('actions.changeLeaveStatusAction: action '. $changedActions[$k]);
//						
//					}
//				}

				$this->getLeaveCompOffService()->changeLeaveCompOffStatus($changedActions);
				$this->getUser()->setFlash('message', __(TopLevelMessages::UPDATE_SUCCESS));
				$this->getUser()->setFlash('messageType', 'success');
			}
