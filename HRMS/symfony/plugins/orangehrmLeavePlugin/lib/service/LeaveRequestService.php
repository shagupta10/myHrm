...
class LeaveRequestService extends BaseService {
    ...
    public function getWFHMonthlyBalance($leaveStartDate, $leaveEndDate ,$empId){
    	$leaveTypeService = $this->getLeaveTypeService();
    	$leaveType = $leaveTypeService->readLeaveTypeByName(LeaveType::LEAVE_TYPE_WFH);
    	if(!is_null($leaveType)){
    		return $this->getLeaveRequestDao()->getWFHMonthlyBalance($leaveStartDate, $leaveEndDate ,$empId,$leaveType->getId());
    	}
    	return true;
    }
    ...
    public function getLeaveByDateAndEmpNumber($empNumber, $date=false) {
    	return $this->getLeaveRequestDao()->getLeaveByDateAndEmpNumber($empNumber, $date ,$isHalfLeave);
    }
}
