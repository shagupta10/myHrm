...
class HolidayDao extends BaseDao {
    ...
    public function searchHolidays($startDate = null, $endDate = null, $offset = 0, $limit = 10,$getCount = false) {
    ...
    public function getUpcomingPublicHolidayList(){
    	try{
    		$q  ="SELECT oh.description,oh.date,oh.recurring FROM ohrm_holiday AS oh";  
    		$q .=" WHERE ( oh.date BETWEEN CURDATE() AND CONCAT(YEAR(CURDATE())+1,'-03-31') ) OR"; 
 			$q .=" ( DATE_ADD(oh.date, INTERVAL YEAR(CURDATE())-YEAR(oh.date) YEAR) BETWEEN CURDATE() AND CONCAT(YEAR(CURDATE())+1,'-03-31')  AND `recurring` = 1)";
    		$pdo = Doctrine_Manager::connection()->getDbh();
    		$res = $pdo->query($q);
    		$upcomingHolidays = $res->fetchAll();
