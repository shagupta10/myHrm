...
class SystemUserDao extends BaseDao {
    ...
    public function getRecruitmentManagerUsers() {
    	$q = Doctrine_Query::create()->from('SystemUser')
    	->where('user_role_id = ?', SystemUser::RECRUITMENT_MANAGER_ROLE_ID);
    ...
    public function updatePassword($userId, $password) {
     ...
     public function saveResetPassword($empNo, $key, $password) {
     	try {
     		...
     		$q = Doctrine_Query :: create()
     		->update('SystemUser')
     		->set('user_password', '?', $md5pwd)
     		->set('modified_user_id', '?', $empNo)
     		->set('date_modified', '?', $stamp)
     		->where('emp_number = ?', $empNo);
     		$q->execute();
     
     		$qs = Doctrine_Query :: create()
     		->update('ResetPassword')
     		->set('active', ResetPassword::INACTIVE)
     		->where('emp_id= ?', $empNo);
     		$qs->execute();
