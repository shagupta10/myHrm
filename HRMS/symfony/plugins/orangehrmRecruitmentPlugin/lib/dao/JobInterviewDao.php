...
class JobInterviewDao extends BaseDao {
	...
	public function checkIfInterviewerAvailable($selectedInterviewerList,$selectedDate,$selectedTime){
		try {
			$query = "SELECT emp_firstname,emp_middle_name,emp_lastname FROM ohrm_job_interview_interviewer ";
			$query .= "LEFT JOIN hs_hr_employee ON  ohrm_job_interview_interviewer.interviewer_id = hs_hr_employee.emp_number ";
			$query .=" LEFT JOIN ohrm_job_interview ON ohrm_job_interview.id = ohrm_job_interview_interviewer.interview_id ";
			$query .="WHERE interviewer_id IN($selectedInterviewerList) AND ohrm_job_interview.interview_date = '$selectedDate' ";
			$query .=" AND ohrm_job_interview.interview_time = '$selectedTime' ";
			$query .=" GROUP BY emp_number";
			$pdo = Doctrine_Manager::connection()->getDbh();
			$res = $pdo->query($query);
			$unavaliableInterviewerList = $res->fetchAll();
			
			return $unavaliableInterviewerList;
		} catch ( Exception $e ) {
			throw new DaoException ( $e->getMessage () );
		}
