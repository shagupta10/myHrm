...
class EmployeeJobDetailsForm extends BaseForm {
    ...
    public function configure() {
        ...
        {
	        $diff = (strtotime(date("Y-m-d")) - strtotime($employee->joined_date));
	        $exp_years   = floor($diff / (365*60*60*24));
	        $exp_months  = floor(($diff - $exp_years * 365*60*60*24) / (30*60*60*24));
	        $syn_exp_month   = (($exp_years*12) + $exp_months);
			$employee->total_experience;
	 		list($before_years,$before_months) = explode(".",$employee->total_experience);

			
			if($before_months!=11 && $before_months!=12)
			{
					$before_months = (substr($before_months,0,1));				
			}
			
			$total_before_exp_months = ($before_years*12 + $before_months);
			$current_exp_months = $total_before_exp_months + $syn_exp_month;

	        $current_exp = floor($current_exp_months/12).'.'.($current_exp_months%12);
        }
        else 
        {
        	$current_exp = $employee->total_experience;
        }
        $this->setDefault('current_experience', $current_exp);
...
//        asort($subUnitList);
    ...
    public function getExistingEmpProjectListAsJson() {
	    ...
	    foreach ($existingProjects as $empProject) {
		    $jsonArray[] = array('name' => $empProject->getProject()->getName(), 'id' => $empProject->getProjectId());
	    }
	    $jsonString = json_encode($jsonArray);
	    return $jsonString;
    }
