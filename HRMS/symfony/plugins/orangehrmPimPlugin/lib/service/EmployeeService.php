...
class EmployeeService extends BaseService {
    ...
    public function getNationalityService() {
    	if(is_null($this->nationalityService)) {
    		...
    		$this->nationalityService->getNationalityDao(new NationalityDao());
    	}
    ...
    public function getCountryService() {
    	if(is_null($this->countryService)) {
    		...
    		$this->countryService->getCountryDao(new CountryDao());
    	}
    ...
    // 
    public function getTodaysBirthday($excludePicture = false){
    	return $this->getEmployeeDao()->getTodaysBirthday($excludePicture);
    }
    /**
    ...
    */
	...
	public function getEmployeeCertifications($empNumber, $certificationCode = null) {
        return $this->getEmployeeDao()->getEmployeeCertifications($empNumber, $certificationCode);
    }
	...
	public function deleteEmployeeCertifications($empNumber, $entriesToDelete = null) {
        return $this->getEmployeeDao()->deleteEmployeeCertifications($empNumber, $entriesToDelete);
    }
	...
	public function saveEmployeeCertification(EmployeeCertification $certification) {
        return $this->getEmployeeDao()->saveEmployeeCertification($certification);
    }
    ...
    public function getEmployeeProjects($empNumber, $projectId = null) {
	    return $this->getEmployeeDao()->getEmployeeProjects($empNumber, $projectId);
    }
     ...
     * @return <type>
     */
    public function getEmployeeCustomerJson($empNumber, $customerId = null) {
	    $jsonArray = array();
	    $customers = $this->getEmployeeCustomer($empNumber, $customerId);
	    foreach ($customers as $customer) {
		    $jsonArray[] = array('name' => $customer->getCustomer()->getName(), 'id' => $customer->getCustomerId());
	    }
    ...
    /**
     * Customer details by Employee
     */
    public function getEmployeeCustomer($empNumber, $customerId = null){
    	return $this->getEmployeeDao()->getEmployeeCustomer($empNumber, $customerId);
    }
    
    /**
     * Save employee project Details
     * @param EmployeeProject $empProject
     * @returns EmployeeProject
     */
    public function saveEmployeeProject(EmployeeProject $empProject) {
	    return $this->getEmployeeDao()->saveEmployeeProject($empProject);
    }
    ...
    public function deleteEmployeeProject($empNumber, $entriesToDelete = null) {
	    return $this->getEmployeeDao()->deleteEmployeeProject($empNumber, $entriesToDelete);
    }
    ...
    public function getEmployeePropertyList($properties, $orderField, $orderBy, $excludeTerminatedEmployees = false, $excludeConsultant = false) {
        return $this->getEmployeeDao()->getEmployeePropertyList($properties, $orderField, $orderBy, $excludeTerminatedEmployees, $excludeConsultant);
    }
    ...
    public function getEmployeeCount($includeTerminated = false, $includeConsultant = false) {
        return $this->getEmployeeDao()->getEmployeeCount($includeTerminated = false, $includeConsultant= false);
    }
    ...
    */
    ...
    public function getSubordinateIdListBySupervisorId($supervisorId, $includeChain = null, $maxDepth = NULL) {
 	...
 	public function getSubordinateIdListBySupervisorIdIncludingSelf($supervisorId,$empNumber,$getAllowedEmployees=false) {
        $configService = $this->getConfigurationService();
        $includeChain = $configService->isSupervisorChainSuported();
        $subordinatelist =  $this->getEmployeeDao()->getSubordinateIdListBySupervisorId($supervisorId, $includeChain, array(), null);
        array_push($subordinatelist, $supervisorId);
        if($getAllowedEmployees) {
        	return $subordinatelist;
        } else {
	        if(in_array($empNumber, $subordinatelist)) {
	        	return true;
	        } else {
	        	return false;
	        }
   ...
   /**
    ...
    public function searchEmployeesForDirectory(EmployeeSearchParameterHolder $parameterHolder) {
    	return $this->getEmployeeDao()->searchEmployeesForDirectory($parameterHolder);
    }
    ...
    public function getSearchEmployeeCount(array $filters = null, $countForDirectory = false) {
        return $this->getEmployeeDao()->getSearchEmployeeCount($filters, $countForDirectory);
    }
    
    public function updateEmployee($emp) {
    	return $this->getEmployeeDao()->updateEmployee($emp);
    }

    public function checksEmployeeExists($empid, $email) {
    	return $this->getEmployeeDao()->checksEmployeeExists($empid, $email);
    }
    
    public function getDirectSupervisors($empNumber) {
    	return $this->getEmployeeDao()->getDirectSupervisors($empNumber);
    }
    
    public function compareDetails($newEmployee, $oldEmployee) {
  		$fieldsMapping = array('emp_personal_email' => 'Personal Email','emp_skype_id' => 'Skype ID','emp_phone_accesscode' => 'Access Code','permanentAddress' => 'Permanent Address','emp_oth_email' => 'Other Email',
    			'emp_work_email' => 'Work Email','emp_mobile' => 'Mobile','emp_hm_telephone' => 'Telephone Number','emp_zipcode' => 'Zipcode','province' => 'Province',
    			'country' => 'Country','city' => 'City','street2' => 'Street2','street1' => 'Street1','emp_work_telephone' => 'Work Telephone','firstName' => 'First Name',
  				'middleName' => 'Middle Name','lastName' => 'Last Name','nickName' => 'Nick Name','fatherName' => 'Father Name','husbandName' => 'Spouse Name','bloodGroup' => 'Blood Group','linkedInUrl' => 'Linked-In URL',
  				'nation_code' => 'Nationality','otherId' => 'Other ID','emp_marital_status' => 'Marital Status','smoker' => 'Smoker','emp_gender' => 'Gender','emp_dri_lice_exp_date' => 'Drv. Lic. Expiry Date','militaryService' => 'Military Service','employeeId' => 'Employee ID',
  				'ssn' => 'SSN','sin' => 'SIN','emp_birthday' => 'Birthday','licenseNo' => 'License No.');
  		$message = "";
  		$changedFields = array();
  		foreach ($fieldsMapping as $p => $v) {
  			
   			if($p=='nation_code')
  			{
  				if($newEmployee[$p]!=$oldEmployee[$p]) {
  					$new = $this->getNationalityService()->getNationalityById($newEmployee[$p]);
  					$old = $this->getNationalityService()->getNationalityById($oldEmployee[$p]);
  					$newEmployee[$p] = $new['name'];
  					$oldEmployee[$p] = $old['name'];
  				}
  			}
  			if($p=='emp_gender')
  			{
  				if($newEmployee[$p]=='1') $newEmployee[$p]='Male';
  				if($newEmployee[$p]=='2') $newEmployee[$p]='Female';
  				if($oldEmployee[$p]=='1') $oldEmployee[$p]='Male';
  				if($oldEmployee[$p]=='2') $oldEmployee[$p]='Female';
  				if(empty($newEmployee[$p])) $newEmployee[$p]='None';
  				if(empty($oldEmployee[$p])) $oldEmployee[$p]='None';
  			}
  			if($p=='country')
  			{
  				if($newEmployee[$p]=='0' && $oldEmployee[$p]=='')
  					$newEmployee[$p] = $oldEmployee[$p];
  				if ($newEmployee[$p] != $oldEmployee[$p])
  				{
  					$new = $this->getCountryService()->getCountryNameById($newEmployee[$p]);
  					$old = $this->getCountryService()->getCountryNameById($oldEmployee[$p]);
  					$newEmployee[$p] = $new['cou_name'];
  					$oldEmployee[$p] = $old['cou_name'];
  				}
  			}

  			//compare 
  			if($newEmployee[$p] != $oldEmployee[$p]) {
  				array_push($changedFields, $p);
  			} 
  		}
  		
		//creates table's row string for an email
  		if(count($changedFields)>0)
  		{
  			foreach ($changedFields as $field)
  			{
  				$fieldName = $fieldsMapping[$field];
  				if(trim($oldEmployee[$field])=='') $oldEmployee[$field]='-------';
  				if(trim($newEmployee[$field])=='') $newEmployee[$field]='-------';
  				$message.="<tr><td>".$fieldsMapping[$field]."</td><td>".$oldEmployee[$field]."</td><td>".$newEmployee[$field]."</td></tr>";
  			}
			$mailer = new PimMailer();
			$mailer->send($newEmployee->getFirstAndLastNames(), $newEmployee->getEmployeeId(), $message);
  		}
    }   
    public function getCurrentProjectTeamMember($empNumber,$CurrentCycleObj) {
        return $this->getEmployeeDao()->getCurrentProjectTeamMember($empNumber,$CurrentCycleObj);
    }
    public function getEmployeeProjectHistory($empNumber,$projectId) {
        return $this->getEmployeeDao()->getEmployeeProjectHistory($empNumber,$projectId);
    }
       /**
     * Get the project list
     * @return <type>
     */
    public function getEmployeeProjectJson($empNumber, $projectId = null) {
	    $jsonArray = array();
	    $projects = $this->getEmployeeProject($empNumber, $projectId);
	    foreach ($projects as $project) {
		    $jsonArray[] = array('name' => $project->getProject()->getName(), 'id' => $project->getProjectId());
	    }
	    $jsonString = json_encode($jsonArray);
	    return $jsonString;
    }
    ...
    /**
     * Project details by Employee
     */
    public function getEmployeeProject($empNumber, $projectId = null){
    	return $this->getEmployeeDao()->getEmployeeProject($empNumber, $projectId);
    }
