<?php

class CommonMessages {
   
    const REQUIRED_FIELD = "Required field";
    const DELETE_CONFIRMATION = "Delete records?";
    const FILE_LABEL_DOC = "Accepts .docx, .doc, .odt, .pdf, .rtf, .txt up to 1MB";
    const FILE_LABEL_IMAGE = "Accepts jpg, .png, .gif up to 1MB";
    const FILE_LABEL_SIZE = "Accepts up to 1MB";
    const DONT_HAVE_ACCESS = "You don't have access to view this";
    const CREDENTIALS_REQUIRED = "Credentials Required";
    const EXPAND_OPTIONS = "Expand for Options";
    const HIDE_OPTIONS = "Hide Options";
    const TOGGABLE_DEFAULT_MESSAGE = self::HIDE_OPTIONS;
    const LABEL_PROCESSING = "Processing";
    const NAME_VALIDATION = "Full Name should contains only alphabates.";
    const ADD_MULTIPLE_VACANCY_CONFIRMATION = "This candidate has already existed in the database, Do you want to link selected vacancy with this candidate?";
}
