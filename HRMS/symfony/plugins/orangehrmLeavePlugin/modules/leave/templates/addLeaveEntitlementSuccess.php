        ...
        <form id="frmLeaveEntitlementAdd" name="frmLeaveEntitlementAdd" method="post" action="">
            ...
            <fieldset>                
                <ol>
                    ...
                    <li>
                    	<label>&nbsp</label>
                      <a href="javascript:void(0)" id="linkCalculateBalance" onClick="calculateLeaveBalance()">Calculate Balance</a>
                    </li>
...
<div class="modal hide" id="noselection">
  <div class="modal-header">
    ...
    <h3><?php echo 'SynerzipHRM - ' . __('No matching employees'); ?></h3>
  </div>
...
<!-- No Employee selected: Begins -->
<div class="modal hide" id="noemployee">
  <div class="modal-header">
    <a class="close" data-dismiss="modal">×</a>
    <h3><?php echo 'SynerzipHRM - ' . __('No employee selected'); ?></h3>
  </div>
  <div class="modal-body">
    <p><?php echo __('Please select the employee to calculate leave entitlement'); ?></p>
  </div>
  <div class="modal-footer">
    <input type="button" class="btn" data-dismiss="modal" id="dialogDeleteBtn" value="<?php echo __('Ok'); ?>" />
  </div>
</div>
<!-- No Employee selected: End -->

<!-- No Joining date : Begins -->
<div class="modal hide" id="nojoiningdate">
  <div class="modal-header">
    <a class="close" data-dismiss="modal">×</a>
    <h3><?php echo 'SynerzipHRM - ' . __('No Joining Date'); ?></h3>
  </div>
  <div class="modal-body">
    <p><?php echo __('Please update the Joining Date of employee to calculate leave entitlement'); ?></p>
  </div>
  <div class="modal-footer">
    <input type="button" class="btn" data-dismiss="modal" id="dialogDeleteBtn" value="<?php echo __('Ok'); ?>" />
  </div>
...
<div class="modal hide" id="preview" style="width:500px">
  <div class="modal-header">
    <a class="close" data-dismiss="modal">×</a>
    <h3><?php echo 'SynerzipHRM - ' . __('Matching Employees'); ?></h3>
  </div>
  <div class="modal-body">
...
<div class="modal hide" id="employeeEntitlement" style="width:500px">
  <div class="modal-header">
    <a class="close" data-dismiss="modal">×</a>
    <h3><?php echo 'SynerzipHRM - ' . __('Updating Entitlement'); ?></h3>
  </div>
  <div class="modal-body">
...
<script type="text/javascript">
    ...
    /*Added by Sujata */
    var lang_check_entitlement = '<?php echo __("Entitlement exceeds the default value"); ?>';
    var getEmployeeJoiningDateUrl = '<?php echo url_for('leave/getEmployeeJoiningDateAjax');?>';
    var leaveTypeEntitlementMapDB = <?php echo $form->getEntitlementDaysMap();?>;
    var wfhDBId='<?php echo LeaveType::LEAVE_TYPE_WFH_ID;?>';
    var plDBId='<?php echo LeaveType::LEAVE_TYPE_PAID_LEAVE_ID;?>';
    
    $(document).ready(function() {
      /*added by sujata for on load event of apply leave form, in order to get joining date */
      if($( "#entitlements_employee_empId" ).val() != ''){
        var empNumber = $('#entitlements_employee_empId').val();
        getEmployeeJoiningDate(empNumber);
      }

      /*added by sujata for onchange event of leave type in order to change entitlement  	*/
      $( "#entitlements_leave_type" ).change(function() {
        var entitlement = getEntitlement();
        $('#entitlements_entitlement').val(entitlement);
      });
      /*added by sujata for onchange event of leave period in oder to change entitlement  	 */
