...
class LeavePeriodService extends BaseService {
    ...
    public function getCurrentAndFutureLeavePeriodByDate($date, $forceReload = false){
        $matchLeavePeriod = array();
        $this->leavePeriodList = $this->getGeneratedLeavePeriodList(null, $forceReload);
       ...
       $currentDate = new DateTime($date);
        ...
        foreach( $this->leavePeriodList as $leavePeriod){
            ...
            $endDate = new DateTime($leavePeriod[1]);
            if(($startDate <= $currentDate) && ($currentDate <= $endDate)){
                $matchLeavePeriod['current'] = $leavePeriod;               
                
            }
             
            if(($startDate > $currentDate) && ($currentDate < $endDate)){
                $matchLeavePeriod['future'] = $leavePeriod;               
                
            }
            
        }
        $SuccessiveleavePeriod=array();
        $SuccessiveleavePeriod[0]=$matchLeavePeriod['current'][0];
        $SuccessiveleavePeriod[1]=$matchLeavePeriod['future'][1];
        
        return $SuccessiveleavePeriod;
    }
