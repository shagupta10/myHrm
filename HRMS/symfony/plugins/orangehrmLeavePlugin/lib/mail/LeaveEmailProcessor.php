...
class LeaveEmailProcessor implements orangehrmMailProcessor {
    ...
    public function getSupervisors($empNumber) {
        $recipients = array();
        $supervisors = $this->getEmployeeService()->getDirectSupervisors($empNumber);
        if (count($supervisors) > 0) {
            foreach ($supervisors as $supervisor) {
                $supervisorDetails = $this->getEmployeeService()->getEmployee($supervisor[0]);
                $to = $supervisorDetails->getEmpWorkEmail();    
                if (!empty($to)) {
                    $recipients[] = $supervisorDetails;
                }
            }
        }
        return $recipients;
