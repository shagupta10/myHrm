...
class ProjectForm extends BaseForm {
	...
	public function configure() {
		...
		for ($i = 1; $i <= $this->numberOfProjectAdmins; $i++) {
			$this->setWidget('projectAdmin_' . $i, new  ohrmWidgetEmployeeNameAutoFill(array('jsonList'=>$this->getEmployeeListAsJson())));
			$this->setWidget('projectAdminType_' . $i, new sfWidgetFormSelect(array('choices' => ProjectAdmin::$projectAdminTypes)));
		}
		...
		for ($i = 1; $i <= $this->numberOfProjectAdmins; $i++) {
			$this->setValidator('projectAdmin_' . $i, new ohrmValidatorEmployeeNameAutoFill(array('required' => false)));
			$this->setValidator('projectAdminType_' . $i, new sfValidatorString(array('required' => false)));
		}
	...
	private function setDefaultValues($projectId) {
		...
		for ($i = 1; $i <= count($admins); $i++) {
			$projectAdminArray = array(
				'empName' => $admins[$i - 1]->getEmployee()->getFirstAndLastNames(),
				'empId' => $admins[$i - 1]->getEmployee()->getEmpNumber()
			);
			
			$this->setDefault('projectAdmin_' . $i, $projectAdminArray);
			$this->setDefault('projectAdminType_' . $i, $admins[$i - 1]->getAdminType());
		}
	...
	public function save() {
		...
		if (empty($id)) {
			...
			$projectAdmins = array();
			for ($i=1; $i<= $this->numberOfProjectAdmins; $i++){
				$projectAdmin = $this->getValue('projectAdmin_' . $i);
				$projectAdmins[$projectAdmin['empId']] = $this->getValue('projectAdminType_' . $i);
			}
			...
			if ($existingProjectAdmins[0]->getEmpNumber() != "") {
				foreach ($existingProjectAdmins as $existingProjectAdmin) {
					...
					if(!array_key_exists($id,$projectAdmins)){
						$existingProjectAdmin->delete();
					}else{
						$existingProjectAdmin->setProjectId($projectId);
						$existingProjectAdmin->setEmpNumber($id);
						$existingProjectAdmin->setAdminType($projectAdmins[$id]);
						$existingProjectAdmin->save();
						unset($projectAdmins[$id]);	
					}
				}
			}
	...
	protected function saveProjectAdmins($projectAdmins, $projectId) {
		foreach ( $projectAdmins as $key => $value ) {
			if(!empty($key)){
				...
				$projectAdmin->setEmpNumber($key);
				$projectAdmin->setAdminType($value);
				$projectAdmin->save();	
			}
		}
	}
