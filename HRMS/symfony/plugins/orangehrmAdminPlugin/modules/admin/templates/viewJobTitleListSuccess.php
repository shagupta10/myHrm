...
<script type="text/javascript"> var recordsPerpage = '<?php echo $recordsLimit; ?>'; </script>
	<?php if($saveMsg) { ?>
	<?php $url = url_for('performance/copyKpi');?>
	<div class = "box">
		<div class = "inner">
     		<div class="message warning nofade">
		     	<?php echo __('Added new Job Title. Due to newly added Job title, Reviews will not be copied from previous Cycles.
		            Do you wish to') ?> <a href = "<?php echo $url ?>?id=<?php echo __($id)?>">Copy Kpi</a>
     		</div> <!-- message warning -->
     	</div>
	</div>
	<?php } ?> 
...
<form name="frmHiddenParam" id="frmHiddenParam" method="post" action="<?php echo url_for('admin/viewJobTitleList?recordsPerPage_Limit='.$recordsLimit); ?>">
